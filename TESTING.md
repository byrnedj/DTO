...
comparing a **baseline** build (from the `origin/new_baseline` branch)
against the **current** working tree, both statically linked into a single
...
1. **Build phase** — CMake compiles two object files:
   ...
   - `dto_baseline.o` from `origin/new_baseline` via `tests/build_baseline.sh`
     (`bl_memcpy`, `bl_memset`, ...)
...
The baseline is built automatically from `origin/new_baseline` during
`cmake --build`. The build script (`tests/build_baseline.sh`):

- Fetches the remote branch and checks the SHA
