...
int thread_func(void *thr_data)
{
    ...
    uint8_t *overlaping_inds= p->overlaping_inds;
    ...
    uint8_t** overlapping_dst_buffs = p->overlapping_dst_buffs;
    ...
    for (unsigned long long i=0; i < max_iters; ++i) {
        ...
        if (overlapping)
            d = overlapping_dst_buffs[i%num_mem_bufs];
        else
        ...
        switch (mem_op) {
            ...
            case MEMCOPY: {
                memcpy(d, s, size);
                printf("dto-test memcpy %d %x %x %d\n",size,s,d,overlapping);
                break;
            }
        ...
        if (sleep_time_us > 0) {
            ...
            //delay = _rdtsc() + sleep_time_cycles;
            //_tpause( 0 , delay);
...
int main(int argc, char **argv)
{
    ...
    float move_probability = 0.0;
    float nonmove_probability = 0.0;
	...
	if (argc != 16) {
		printf("Usage: dto-test-distribution num_threads sleep_time num_iters mem_buf_size num_mem_bufs use_random_index src_numa_policy dst_numa_policy num_burst_threads burst_size time_between_ms warmup_time_s percent_src_dst_overlap overlap_probability input_file_path \n");
		exit(0);
	}

    num_threads = atoi(argv[1]);
    sleep_time_us = atoi(argv[2]);
    sscanf(argv[3], "%llu", &num_iter);
    mem_buf_size = atoi(argv[4]);
	num_mem_bufs = atoi(argv[5]);
    random_access = atoi(argv[6]);
	src_numa_policy = atoi(argv[7]);
	dst_numa_policy = atoi(argv[8]);
    num_burst_threads = atoi(argv[9]);
    sscanf(argv[10], "%llu", &burst_size);
    time_between_ms = atoi(argv[11]);
    warmup_time_s = atoi(argv[12]);
    percent_src_dst_overlap = atoi(argv[13]);
	overlap_probability = atoi(argv[14]);
    ...
    while(fgets(line, 100, fp )!= NULL)
    {
        ...
        nonmove_probability += set_probs[count];

        sp = strtok(NULL, ",");
        cpy_probs[count] = atof(sp);
        nonmove_probability += cpy_probs[count];

        sp = strtok(NULL, ",");
        mov_probs[count] = atof(sp);
        move_probability += mov_probs[count];
    ...
    if (move_probability == 0.0 && overlap_probability != 0) {
        printf("overlaping buffers supported only for MEMMOVE. The distirbution file contains no move operations overlap_probablity is set to 0.\n");
    ...
    uint8_t *overlaping_inds = calloc(index_length, sizeof(uint8_t));
    double rn;
    int ind;
	...
	for (uint32_t i=0; i < index_length; ++i) {
        ...
        // no overlap
        if (entries[ind].op != MEMMOVE || overlap_probability == 0 || getRand2(1) > prob) 
            overlaping_inds[i] = 0;
        else
            overlaping_inds[i] = 1;
    }
    ...
    if (overlap_probability < 100 || nonmove_probability > 0.0) {
    ...
    if (overlap_probability > 0) {
        non_overlap_size = max_transaction_size*(100-percent_src_dst_overlap)/100;
        t_size_overlap = max_transaction_size + non_overlap_size;
    }
    ...
    uint8_t** overlapping_dst_addrs = calloc(ind_size, sizeof(uint8_t*));
    if (num_mem_bufs>1) {
		...
		for (uint32_t i=0;i<ind_size;++i) {
            ...
            if (overlap_probability == 0) {
                ...
                overlapping_dst_addrs[i] = src_addrs[i] + non_overlap_size*BUF_SIZE_BASE;
            }
        }
	}
    ...
    if (random_access) {
        for (uint32_t i=ind_size-1;i>0;--i) {
            ...
            swap(&overlapping_dst_addrs[i],&overlapping_dst_addrs[j]);
        }
    }
	...
	for(int t = 0; t < num_threads; ++t) {
        ...
        p[t].overlapping_dst_buffs = &overlapping_dst_addrs[t*bufs_per_thread];
