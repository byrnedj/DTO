...
static atomic_uint_fast16_t latest_updates[2*MAX_AUTOTUNE_INSTANCES][NUM_LATEST_UPDATES];
			...
			if (make_adjustments) {
				if (avg_num_waits > max_avg_waits) {
					if (auto_tune_states[bucket].cpu_size_fraction < MAX_CPU_SIZE_FRACTION) {
						...
						//printf("avg num waits %d new cpu fract at %d increased min size\n",avg_num_waits,auto_tune_states[bucket].cpu_size_fraction);
						ut = INCR_MIN_SIZE;
					}
					...
					if (auto_tune_states[bucket].cpu_size_fraction >= CSF_STEP_DECREMENT) {
						...
						//printf("avg num waits %d new cpu fract at %d decreased min size\n",avg_num_waits,auto_tune_states[bucket].cpu_size_fraction);
						ut = DECR_MIN_SIZE;
					}
				}
...
static int init_dto(void)
{
	...
	if (atomic_compare_exchange_strong(&dto_initializing, &init_notcomplete, 1)) {
		...
		if (collect_alg_stats) {
			for(int j=0;j<2*MAX_AUTOTUNE_INSTANCES;++j)
			{
		...
		if (!use_std_lib_calls) {
			...
			for(int i=0; i < dto_num_autotune_instances; ++i) {
				auto_tune_states[i].cpu_size_fraction = cpu_size_fraction;
				auto_tune_states[i].wq_index_offset = i*num_wqs_per_bucket;
				auto_tune_states[i].num_wqs = num_wqs_per_bucket;
				auto_tune_states[i].next_wq = 0;

				auto_tune_states[i+MAX_AUTOTUNE_INSTANCES].cpu_size_fraction = cpu_size_fraction;
				auto_tune_states[i+MAX_AUTOTUNE_INSTANCES].wq_index_offset = i*num_wqs_per_bucket;
				auto_tune_states[i+MAX_AUTOTUNE_INSTANCES].num_wqs = num_wqs_per_bucket;
				auto_tune_states[i+MAX_AUTOTUNE_INSTANCES].next_wq = 0;
			}
