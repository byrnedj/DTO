...
int main(int argc, char **argv)
{
	...
	if (argc < 11) {
		printf("Usage: dto-test-settable-size num_threads sleep_time_us mem_op buf_size max_iters mem_buf_size num_mem_bufs use_random_index src_numa_policy dst_numa_policy [buf_size, max_iters for remaining threads]\n");
		...
		if(p[0].num_threads>1 && argc>11) {
			if(argc != 11+(p[0].num_threads*2)) {
			...
			for(int i = 1; i<p[0].num_threads;++i){
				...
				p[i].buf_size = atoi(argv[11+(i-1)*2]);
				p[i].max_iters = atoi(argv[12+(i-1)*2]);
