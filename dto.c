	...
	if (atomic_compare_exchange_strong(&dto_initializing, &init_notcomplete, 1)) {
		...
		if (!use_std_lib_calls) {
			...
			LOG_TRACE("log_level: %d, collect_stats: %d, use_std_lib_calls: %d, dsa_min_size: %lu, "
				...
				"wait_method: %s, auto_adjust_knobs: %d, numa_awareness: %s, dto_dsa_cc: %d, dto_dsa_batch: %d, dto_dsa_bof: %d, "
				...
				cpu_size_fraction[TUNE_CTX(SIZE_CLASS_LARGE, CACHE_IN)]/100.0,
				cpu_size_fraction[TUNE_CTX(SIZE_CLASS_LARGE, CACHE_OUT)]/100.0,
				wait_names[wait_method], auto_adjust_knobs, numa_aware_names[is_numa_aware], dto_dsa_cc, dto_dsa_batch, dto_dsa_bof,
...
static bool dto_memcpymove(void *dest, const void *src, size_t n, bool is_memcpy, int *result)
{
	...
	if (dsa_size <= wq->max_transfer_size) {
		...
		thr_comp.status = 0;
		if (is_overlapping) {
			*result = dsa_execute(wq, &thr_desc, &thr_comp.status, ctx);
		} else {
			*result = dsa_submit(wq, &thr_desc);
			if (*result == SUCCESS) {
				if (cpu_size) {
