...
name = 'sweepalltest4_{}_{}'.format(run_name, output_type)
                            ...
                            for cache_control in cache_control_configs:
                                ...
                                for perc in percentages:
                                    ...
                                    for size in sizes:
                                        ...
                                        output_name = '{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}K'.format(op_name,num_threads,numa_config,perc.replace(".", "-"),wait,alloc,cache_control,r,num_b,mem_buf_size,size)
                                        ...
                                        if alloc == 'single':
                                            ...
                                            if distribution_filepath is None:
                                                dto_command = dto_command_base + [str(num_threads), str(0), str(mem_op), str(size), str(num_iter), str(alloc_size), '1', str(r), numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst]]
                                            else:
                                                dto_command = dto_command_base + [str(num_threads), str(0), str(num_iter), str(alloc_size), '1', str(r),numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst], distribution_filepath]
                                        else:
                                            if distribution_filepath is None:
                                                dto_command = dto_command_base + [str(num_threads), str(0), str(mem_op), str(size), str(num_iter), str(mem_buf_size), str(num_b), str(r),numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst]]
                                            else:
                                                dto_command = dto_command_base + [str(num_threads), str(0), str(num_iter), str(mem_buf_size), str(num_b), str(r),numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst], distribution_filepath]
