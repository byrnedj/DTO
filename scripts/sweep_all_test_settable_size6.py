...
dto_command_base_settable = ['../dto-test-settable-size-dev6']
dto_command_base_distribution = ['../dto-test-distribution-multithread-dev6']
...
mem_op_names = {MEMSET:'set', MEMCOPY:'cpy'} #, 'mov']
...
#TODO: add ramp types and ramp modes to config, add them to the sweep...
ramp_type = 0
ramp_mode = 0
...
if cfg_filepath is not None:
    ...
    if "sleep_delays" in loaded_config:
        sleep_delays = config["sleep_delays"] = loaded_config["sleep_delays"]

    if "ramp_modes" in loaded_config:
        ramp_modes = config["ramp_modes"] = loaded_config["ramp_modes"]

    if "ramp_types" in loaded_config:
        ramp_types = config["ramp_types"] = loaded_config["ramp_types"]
...
name = 'all6_{}_{}'.format(run_name, output_type)
...
for num_threads in thread_counts:
    ...
    for num_bursting_threads in bursting_thread_counts:
        for burst_size in burst_sizes:
            for time_between_bursts in time_between_bursts_ms:
                                                    ...
                                                    for sleep_delay in sleep_delays:
                                                        ...
                                                        for perc in percentages:
                                                            ...
                                                            for size in sizes:
                                                                ...
                                                                output_name = '{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}K'.format(op_name,num_threads,sleep_delay,ramp_type,ramp_mode,num_bursting_threads,burst_size,time_between_bursts,warmup_time_s,map_numa_config[numa_config],perc.replace(".", "-"),wait,alloc,cache_control,r,num_b,mem_buf_size,size)
                                                                ...
                                                                if alloc == 'single':
                                                                    ...
                                                                    if distribution_filepath is None:
                                                                        dto_command = dto_command_base + [str(num_threads), str(sleep_delay), str(ramp_type), str(ramp_mode), str(mem_op), str(size), str(num_iter), str(alloc_size), '1', str(r), numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst], str(num_bursting_threads), str(burst_size), str(time_between_bursts), str(warmup_time_s)]
                                                                    else:
                                                                        dto_command = dto_command_base + [str(num_threads), str(sleep_delay), str(ramp_type), str(ramp_mode), str(num_iter), str(alloc_size), '1', str(r),numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst], str(num_bursting_threads), str(burst_size), str(time_between_bursts), str(warmup_time_s), distribution_filepath]
                                                                else:
                                                                    if distribution_filepath is None:
                                                                        dto_command = dto_command_base + [str(num_threads), str(sleep_delay), str(ramp_type), str(ramp_mode), str(mem_op), str(size), str(num_iter), str(mem_buf_size), str(num_b), str(r),numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst], str(num_bursting_threads), str(burst_size), str(time_between_bursts), str(warmup_time_s)]
                                                                    else:
                                                                        dto_command = dto_command_base + [str(num_threads), str(sleep_delay), str(ramp_type), str(ramp_mode), str(num_iter), str(mem_buf_size), str(num_b), str(r),numa_alloc_policy[numa_alloc_policy_src], numa_alloc_policy[numa_alloc_policy_dst], str(num_bursting_threads), str(burst_size), str(time_between_bursts), str(warmup_time_s), distribution_filepath]
