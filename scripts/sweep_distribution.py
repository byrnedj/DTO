...
dto_command = dto_command_base + [str(num_iter), './all_ops.csv']
if run_perf:
    dto_command = perf_command + dto_command
...
for min_size in min_sizes:
    ...
    fn= 'results_{}_{}K_nodsa.txt'.format(name, min_size)
    print("processing {}K no DSA".format(min_size))
    ...
    for perc in percentages:
        if perc == 'auto':
            ...
            print("processing {}K auto cpu fraction".format(min_size))
            
        else:
            dto_env['DTO_AUTO_ADJUST_KNOBS']='0'
            dto_env['DTO_CPU_SIZE_FRACTION']=str(perc)
            print("processing {}K {} cpu fraction".format(min_size,perc))

        fn= 'results_{}_{}K_{}.txt'.format(name, min_size,perc)
