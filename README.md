...
The following features have been added to the dev-dto library (see below for related ENV variables):
   - Within the stats collection 
      - Bytes completed on CPU is counted separately and reported in the various groups. Specifically in DSA success group bytes on CPU are the bytes perfored by CPU according to the CPU fraction split.
      - Collection of NUMA node of source/destination for memset and memcpy. We count the number of occurances for src/destination for memset/memcpy for each numa node.
      - WQ occupancy stats for each WQ in the form of a histogram (number of requests in queue at the time of descriptor submission)
   - DTO stats output in the form of a Python dictionary (similar to json) to allow for easier parsing of DTO stats output.
   - Added a third logging level to separate out Trace logging from stats logging. 
   - Collection of stats related to autotune algorihthm. The following stats are collected
      - CPU percentage (fraction) in the form of a histogram and a timeseries of the latest N values
      - Number of wait cycles (number of times around the wait loop) - histogram
      - Update types (which action the tuning algorith took) in the form of a histogram and latest N values.
      - dsa_min_size in the form of a histogram
   - Added the ability to run the autotune algorithm for the purposes of algorithm stats collection without actually making adjustments. (This allows for collecting of stats like number of waits for specified values of CPU fraction.)
   - Ability to set maximum size of transaction for which DTO is used. (This is probably not useful, but its there :)
   - Added a new wait method for investigative purposes which uses Linux usleep call. This is not useful in practive because optimal sleep times depend on tranaction size and Linux sleep is not granular enough.
   - Independant instance of the auto-tune algorithm for:
      - subsets of transaction sizes. (The user specifies how many instances and the size range between min_bytes and max_bytes is equally divided)
      - memset and memcpy - independant instance of algorithm for memset and memcpy
      - The two modes can be combined to get independent instances of algorithm for memset and memcpy for individual transaction size ranges.
   - Ability to use only a subset of configured WQs. This is not very useful because the subset is taken blindly from the start of the list, so the numa node of selected WQs is not controllable.
   - Opposite NUMA awareness mode. This is a version of numa awareness where the WQs are selected from the opposite numa. The feature only works for systems with 2 nodes. Its main purpose is to characterize performance across numa nodes.
