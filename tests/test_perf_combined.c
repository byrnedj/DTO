 ...
 *
 * test_perf_combined.c - Statically-linked A/B performance comparison.
 *
		   ...
		   int iters)
{
	...
	if (pid == 0)
		child_run_ab(test, slot, src, dst, cfg->use_libc);
				   ...
				   const char *results_dir)
{
	...
	for (int r = 0; r < ab_rounds; r++) {
		...
		if (fork_ab(test, slot, src, dst, cfg, iters) < 0)
			goto out;
...
int main(void)
{
		...
		fprintf(stderr, "WARNING: could not pin to CPU %d\n", cpu);
	{
		...
		if (freq_env) {
			if (pin_freq(cpu, strtoul(freq_env, NULL, 10)) < 0) {
				fprintf(stderr,
					"ERROR: PERF_FREQ_MHZ=%s but could not "
					"pin. Run as root.\n", freq_env);
			}
		}
	}
	...
	printf("DTO Combined A/B Performance Test (%s cache)\n",
