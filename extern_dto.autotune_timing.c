...
static void cleanup_dto(void)
{
	...
	float avg_latency = 1.0 * cycles / t_ops;
	uint64_t cycles_per_sec;
	calibrate(&cycles_per_sec);

	secs = (float)cycles/cycles_per_sec;
	float latency_ns = (latency * 1E9)/cycles_per_sec;
	float avg_latency_ns = (avg_latency * 1E9)/cycles_per_sec;
	
	printf("Autotune latency per invocation %f ns average latency %f ns %llu invocation\n",latency_ns, avg_latency_ns, c_ops);
